...
from project_config import DATA, TRAIN_UNET_T2, ensure_project_dirs
...
def main():
    ensure_project_dirs()
    seed_all(TRAIN_UNET_T2["seed"])
    print("******************* train_unet_t2 *******************")
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    data_root = DATA["train_new"]
    model_save = TRAIN_UNET_T2["model_path"]
    os.makedirs(os.path.dirname(model_save), exist_ok=True)
    batch_size = TRAIN_UNET_T2["batch_size"]
    num_workers = TRAIN_UNET_T2["num_workers"]
    lr = TRAIN_UNET_T2["lr"]
    epochs = TRAIN_UNET_T2["epochs"]
